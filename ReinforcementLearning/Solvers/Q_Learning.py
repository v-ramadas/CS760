...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for stepNum in range(self.options.steps):
            action = self.epsilon_greedy_action(state)
            next_state, reward, done, _ = self.step(action)
            best_action = np.argmax(self.Q[next_state])
            self.Q[state][action] += self.options.alpha*(reward + (self.options.gamma*self.Q[next_state][best_action]) - self.Q[state][action])
            state = next_state
            if done is True:
                break
